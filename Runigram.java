...
public class Runigram {
	 ...
	 * of the source image.
	 * Morphing: suppose we want to morph a source image into a target image gradually, in n steps. To do
so, we stage a sequence of 0, 1, 2, ... , n steps, as follows. In each step i we blend the source image and
the target image using α = (n − i)/n. For example, here is what happens when n = 3:
...
The function void morph(Color[][] source, Color[][] target, int n) morphs the source
image into the target image in n steps. If the images don't have the same dimensions, the function
scales the target image to the dimensions of the source image, and then starts the morphing sequence.
At the end of each blending step, the function should use the Runigram.display function to display
the intermediate result, and the StdDraw.pause function to pause for about 500 milliseconds.
